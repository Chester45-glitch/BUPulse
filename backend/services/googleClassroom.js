        ...
        if (w.dueDate) {
          ...
          const dueDate = new Date(
            `${d.year}-${String(d.month).padStart(2,"0")}-${String(d.day).padStart(2,"0")}T${
              w.dueTime ? `${String(w.dueTime.hours||0).padStart(2,"0")}:${String(w.dueTime.minutes||0).padStart(2,"0")}:00` : "23:59:00"
            }`
          );
          all.push({
...
const getAllAnnouncements = async (accessToken, refreshToken) => {
  ...
  return all.sort((a, b) => new Date(b.updateTime) - new Date(a.updateTime));
};
