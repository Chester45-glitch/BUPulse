...
function NavItem({ to, Icon, label, end, expanded, accent, onClick }) {
  return (
    <NavLink to={to} end={end} onClick={onClick} style={{ display: "block", textDecoration: "none" }}>
      {({ isActive }) => (
        <div
          style={{ display: "flex", alignItems: "center", gap: 14, padding: "4px 8px", borderRadius: 14, cursor: "pointer", color: isActive ? "#fff" : "rgba(255,255,255,0.55)", background: isActive ? "rgba(255,255,255,0.08)" : "transparent", borderLeft: `3px solid ${isActive ? accent : "transparent"}`, transition: "all 0.15s ease", marginBottom: 2, overflow: "hidden" }}
          onMouseEnter={(e) => { if (!isActive) e.currentTarget.style.background = "rgba(255,255,255,0.05)"; }}
          onMouseLeave={(e) => { if (!isActive) e.currentTarget.style.background = "transparent"; }}
        >
          <div style={{ width: 44, height: 44, borderRadius: 12, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: isActive ? `${accent}22` : "rgba(255,255,255,0.04)", color: isActive ? accent : "rgba(255,255,255,0.65)", transition: "all 0.15s" }}>
            <Icon />
          </div>
          <span style={{ fontSize: 14, fontWeight: isActive ? 600 : 400, opacity: expanded ? 1 : 0, transition: "opacity 0.18s ease", whiteSpace: "nowrap" }}>
            {label}
          </span>
        </div>
      )}
...
export default function Sidebar({ isOpen, onClose, user, role = "student" }) {
  ...
  const onLeave = useCallback(() => { timer.current = setTimeout(() => setExpandedState(false), 300); }, []);
  ...
  return (
    <>
      ...
      <aside className={`sb-root ${isOpen ? "sb-open" : ""}`} onMouseEnter={onEnter} onMouseLeave={onLeave} aria-label="Main navigation" style={{ "--accent": accent }}>

        {/* Logo */}
        <div style={{ padding: "16px 8px 8px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 10, overflow: "hidden" }}>
          ...
          <div style={{ opacity: effectiveExpanded ? 1 : 0, transform: effectiveExpanded ? "translateX(0)" : "translateX(-8px)", transition: "all 0.18s ease", flex: 1, minWidth: 0 }}>
        ...
        <nav style={{ flex: 1, padding: "10px 8px", overflowY: "auto", overflowX: "hidden" }}>
          {navItems.map((item) => (
            <NavItem key={item.to} {...item} end={item.to === "/professor" || item.to === "/parent"} expanded={effectiveExpanded} accent={accent} onClick={onClose} />
          ))}
        </nav>
        ...
        <div style={{ padding: "8px 8px 14px", borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: "column", gap: 2, flexShrink: 0 }}>
          <button onClick={toggleDark} style={{ display: "flex", alignItems: "center", gap: 14, padding: "4px 8px", borderRadius: 14, background: "transparent", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.5)", width: "100%", overflow: "hidden" }}
          ...
          <button onClick={handleLogout} style={{ display: "flex", alignItems: "center", gap: 14, padding: "4px 8px", borderRadius: 14, background: "transparent", border: "none", cursor: "pointer", color: "rgba(248,113,113,0.65)", width: "100%", overflow: "hidden" }}
      ...
      <style>{`
        .sb-root { width:72px; min-height:100vh; height:100%; position:fixed; left:0; top:0; bottom:0; display:flex; flex-direction:column; background:#111a11; border-right:1px solid rgba(255,255,255,0.06); z-index:50; transition:width 0.25s cubic-bezier(0.4,0,0.2,1); overflow:hidden; }
        .sb-root:hover { width:240px; }
        .sb-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.45); backdrop-filter:blur(2px); z-index:40; display:none; }
