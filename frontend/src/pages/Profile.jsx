...
const ROLE_COLORS = {
  ...
  professor: { bg: "#dbeafe", color: "#1d4ed8", label: "Professor" },
  parent: { bg: "#ede9fe", color: "#7c3aed", label: "Parent" },
};

const InfoRow = ({ label, value, icon }) => (
  <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 0", borderBottom: "1px solid var(--border-color)" }}>
    <div style={{ width: 36, height: 36, borderRadius: "var(--radius-md)", background: "var(--bg-tertiary)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, flexShrink: 0 }}>{icon}</div>
    <div>
...
export default function Profile() {
  ...
  const studentActions = [
    { icon: "📚", label: "Enrolled Classes", sub: "View all your active courses", path: "/enrolled-classes" },
    { icon: "⏳", label: "Pending Activities", sub: "Check what needs to be submitted", path: "/pending-activities" },
    { icon: "📢", label: "Announcements", sub: "Stay up to date", path: "/announcements" },
  ];

  const professorActions = [
    { icon: "📢", label: "Post Announcement", sub: "Notify your students", path: "/professor" },
    { icon: "📚", label: "My Classes", sub: "View courses you teach", path: "/professor" },
  ];

  const parentActions = [
    { icon: "👨‍👩‍👧", label: "Student Dashboard", sub: "Monitor your child's progress", path: "/parent" },
  ];

  const actions = user?.role === "professor" ? professorActions : user?.role === "parent" ? parentActions : studentActions;
    ...
    <div style={{ animation: "fadeIn 0.4s ease", maxWidth: 600 }}>
      ...
      <div style={{ background: "var(--card-bg)", borderRadius: "var(--radius-xl)", border: "1px solid var(--card-border)", overflow: "hidden", boxShadow: "var(--shadow-md)", marginBottom: 20 }}>
        ...
        <div style={{ padding: "0 24px 24px" }}>
          <div style={{ marginTop: -40, marginBottom: 14, display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
            {user?.picture
            ...
            <span style={{ background: roleInfo.bg, color: roleInfo.color, fontSize: 12, fontWeight: 700, padding: "4px 12px", borderRadius: 99 }}>{roleInfo.label}</span>
          </div>
      ...
      <div style={{ background: "var(--card-bg)", borderRadius: "var(--radius-xl)", border: "1px solid var(--card-border)", padding: "20px 24px", boxShadow: "var(--shadow-sm)", marginBottom: 16 }}>
        <h3 style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)", marginBottom: 16 }}>Account Information</h3>
        <InfoRow icon="👤" label="Full Name" value={user?.name} />
        <InfoRow icon="📧" label="Email Address" value={user?.email} />
        <InfoRow icon="🎭" label="Role" value={roleInfo.label} />
