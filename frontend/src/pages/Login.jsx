...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (err === "timeout")
      setError("Sign-in timed out — our server may be warming up. Please try again.");
  ...
  // Reset loading if user comes back to this tab/page (e.g. closed Google OAuth popup,
  // pressed back, or the redirect failed). Without this, the button stays "Redirecting…" forever.
  useEffect(() => {
    const resetOnReturn = () => { setLoading(false); };
    // pageshow fires when navigating back via browser history
    window.addEventListener("pageshow", resetOnReturn);
    // visibilitychange fires when user switches tabs and comes back
  ...
  const handleLogin = () => {
    ...
    // Safety timeout: if we're still on this page after 30s, reset the button
